...
from function.machinelearning import calculateLogo
import json, os
...
def addOrder(request: Request, response: Response, jenisproduk:int, jenisbahan:int, warna:int, jenislogo:int, xxl:int, xl:int, l:int, m:int, s:int, image: UploadFile):
    sessionToken = request.headers.get("Authorization")
    auth = authCheck(sessionToken)
    if not auth["login"]:
        response.status_code = status.HTTP_401_UNAUTHORIZED
    ...
    # Detect logo
    detectionData = calculateLogo(imagePath)
    ...
    # imagePath
    imagePath = os.path.join(os.getenv("BASE_URL"), "static/generated/order/", imageName)
